...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($id) {
        ...
        if (!$old_result || mysqli_num_rows($old_result) == 0) {
            ...
            if ($jumlah < $old_data['jumlah']) {
                // Split: update current to new condition with $jumlah, insert new for remaining
                $remaining_jumlah = $old_data['jumlah'] - $jumlah;
                ...
                // Insert new row for remaining
                $insert_sql = "INSERT INTO barang (nama_barang, ruangan, jumlah, kondisi, status_pengecekan, nama_petugas, tanggal_pengecekan, keterangan)
                                VALUES ('" . mysqli_real_escape_string($conn, $old_data['nama_barang']) . "',
                                        '" . mysqli_real_escape_string($conn, $old_data['ruangan']) . "',
                                        $remaining_jumlah,
                                        '" . mysqli_real_escape_string($conn, $old_data['kondisi']) . "',
                                        '" . mysqli_real_escape_string($conn, $old_data['status_pengecekan']) . "',
                                        '" . mysqli_real_escape_string($conn, $old_data['nama_petugas']) . "',
                                        '" . mysqli_real_escape_string($conn, $old_data['tanggal_pengecekan']) . "',
                                        '" . mysqli_real_escape_string($conn, $old_data['keterangan']) . "')";

                if (mysqli_query($conn, $sql) && mysqli_query($conn, $insert_sql)) {
                    $new_id = mysqli_insert_id($conn); // For logging the new row
                    // Log changes for updated row
                    $fields_to_check = ['jumlah', 'kondisi', 'status_pengecekan', 'nama_petugas', 'tanggal_pengecekan', 'keterangan'];
                    foreach ($fields_to_check as $field) {
                        if ($old_data[$field] != ${$field}) {
                            logChange($id, $field, $old_data[$field], ${$field}, $nama_petugas ?: 'Admin');
                        }
                    }
                    // Log creation of new row
                    logChange($new_id, 'created_from_split', '0', $remaining_jumlah, $nama_petugas ?: 'Admin');
                } else {
            ...
            <form method="POST" action="" style="margin-top: 15px;">
                ...
                <?php elseif ($mode == 'update' && $edit_data): ?>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 15px;">
                        ...
                        <div>
                            <label for="jumlah" style="display: block; margin-bottom: 5px; font-weight: bold;">Jumlah *</label>
                            <input type="number" id="jumlah" name="jumlah" min="1" value="<?php echo $edit_data['jumlah']; ?>" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px;">
                        </div>
